...
- Full local-card accountability by passcode/cardId manifest.
...
- Full 1,704-card effect automation as a blocker for playability.
...
- No Side Deck input.
- No Extra Deck input.
- Deck validation rejects unsupported, deck-blocked, out-of-pool, and forbidden cards in normal playable mode.
...
- Forbidden/limited/semi-limited handling may still exist as metadata, but the immediate playable-mode constraint is:
  - exactly 40 cards
  - only cards with playable manifest status and legal copy/scope status
...
6. Cards requiring Extra/Fusion Deck are `goatDeckBlocked`.
...
Progress:
- DONE: Phase progression, summon/set placement, tribute selection, attacks, LP edits, and dev/debug movement now route through core reducer-backed engine actions from `src/gameLogic.ts` and `src/engine/duel.ts`.
...
- DONE: Hand Graveyard/Banish movement controls are explicit dev-only debug actions and now return explicit unsupported/illegal core results instead of mutating through legacy movement.
...
- DONE: Added a static guard that allows card `text` only in catalog/display normalization files and scans `src/engine/**/*.ts`, `src/cardData.ts`, `src/gameLogic.ts`, and adapter paths.
- DONE: Added a runtime-behavior module check that rejects card-text property access outside allowed catalog/display files; `getSummonProfile` no longer parses card text for summon eligibility.

---

## T-074: Add playable coverage report and accountability manifest

Status: DONE

Goal:
Report what can and cannot be used in playable decks while accounting for every local card by passcode/cardId.

Files:
- `src/engine/cards/coverageManifest.generated.ts`
- `src/engine/cards/coverageReport.ts`
- `src/engine/__tests__/coverageReport.test.ts`
- `src/engine/__tests__/coverageManifest.test.ts`
- `docs/implementation-status.md`

Actions:
1. Report:
   - total local cards
   - count by manifest status
   - GOAT-legal unsupported count
   - GOAT-legal unsupported card list
   - strict final acceptance readiness
...
5. Require every local passcode to have exactly one manifest status.
...
Progress:
- DONE: Added `coverageManifest.generated.ts` with exactly one status for every local `cards.json` passcode.
- DONE: Added `buildPlayableCoverageReport` with totals for local cards, status counts, GOAT-legal unsupported count/list, and `strictFinalAcceptanceReady`.
...
- DONE: Added coverage report and manifest tests proving all local cards are accounted for, strict final acceptance remains false while unsupported GOAT-legal cards remain, scripted statuses have registered scripts/templates, and supported playable deck fixtures contain zero unsupported/deck-blocked/out-of-pool cards.
...
- Queue can be considered complete for simplified playable-engine scope, but not strict full-card-accountability readiness.
...
- DONE: Confirmed the simplified playable-engine scope is complete while strict full-card-accountability readiness remains open.

---

# Phase 8: Full card-accountability readiness

## T-080: Close audit blockers for reducer-routed frontend facade

Status: DONE

Goal:
Make the frontend-compatible path trustworthy by removing mutable gameplay bypasses, preserving determinism, fail-closing broken chain resolution, and adding full manifest accountability scaffolding.
...
Progress:
- DONE: Routed UI-compatible gameplay actions for draw, phase, End Turn, summon/set, Spell/Trap set, activation, attack, priority, chain resolution, and prompts through core reducer commands.
- DONE: Removed runtime engine `crypto.randomUUID()` event generation and added deterministic compatibility event IDs.
- DONE: Added frontend-path Pot of Greed and Mystical Space Typhoon regressions proving implemented scripts resolve through reducer-backed UI helpers/actions.
- DONE: Changed chain resolution so malformed, missing-script, and missing-resolution links emit explicit failure events instead of silently no-oping.
- DONE: Added full local card manifest/accountability tests and strict acceptance reporting.
- DONE: Expanded the no-runtime-card-text parsing guard beyond engine files.

---

## T-081: Complete any remaining frontend/core routing gaps

Status: TODO

Goal:
Keep the compatibility facade as a projection over core state and migrate any future UI action that still mutates gameplay state outside the reducer.

Acceptance:
- New UI gameplay actions have reducer commands or explicit unsupported events.
- Tests prove `coreState` stays synchronized after every supported UI action.

---

## T-082: Implement full SEGOC ordering

Status: TODO

Goal:
Replace simplified trigger ordering with complete simultaneous effects go on chain policy for mandatory/optional effects, turn player/non-turn player ordering, and controller choice ordering where applicable.

Acceptance:
- Golden tests cover representative simultaneous mandatory and optional trigger groups.
- Existing trigger behavior remains deterministic.

---

## T-083: Implement Damage Step subwindows

Status: TODO

Goal:
Expose the Damage Step windows needed for real GOAT timing instead of treating most battle resolution as one atomic reducer action.

Acceptance:
- Tests cover legal/illegal activation windows across Damage Step substeps.
- Supported battle cards still resolve deterministically.

---

## T-084: Implement End Phase ordering

Status: TODO

Goal:
Model End Phase ordering, including Spirit return before hand-size discard, with prompts or deterministic ordering where required.

Acceptance:
- Spirit return occurs before hand-size discard.
- End Phase cleanup, lingering effects, and discard prompts are visible and tested.

---

## T-085: Implement card-family batches

Status: TODO

Goal:
Reduce the `goatUnsupported` count through tested script/template batches grouped by card family and shared behavior.

Acceptance:
- Each batch adds scripts/templates and focused tests.
- Deck validation only admits newly supported cards when legal under manifest, banlist/copy, and scope rules.
- No runtime card-text parsing is introduced.

---

## T-086: Enforce final strict acceptance gate

Status: TODO

Goal:
Make strict final acceptance pass only when every GOAT-legal local card is either implemented/playable, forbidden-but-scripted, or explicitly deck-blocked by the simplified no-Extra/Fusion scope.

Acceptance:
- `goatLegalUnsupportedCount` is zero.
- `strictFinalAcceptanceReady` is true only at zero unsupported GOAT-legal cards.
- Final validation and audit docs prove the result.

---

# Simplified playable acceptance criteria

The simplified playable-engine scope is complete when all are true:
...
# Strict full-card-accountability acceptance criteria

Strict final acceptance is complete only when all are true:

1. Every local `cards.json` passcode has exactly one manifest status.
2. No manifest entry exists for a passcode absent from `cards.json`.
3. No local passcode is duplicated.
4. `goatTemplate`, `goatCustom`, and `goatForbiddenButScripted` entries all have registered scripts/templates.
5. Normal playable deck validation rejects `goatUnsupported`, `goatDeckBlocked`, `notInGoatPool`, and forbidden cards.
6. `goatLegalUnsupportedCount` is zero.
7. `strictFinalAcceptanceReady` is true only because the unsupported count is zero.
8. `npm run typecheck`, `npm test`, and `npm run build` pass.
